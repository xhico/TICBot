...
def csvReader(mailURL):
    ...
    :param: mailURL is string pointing to the URL
...
def checkInfo(curso):
    ...
    Compares dates of all the entries from the server with the latest date stored locally
...
def resetAccounts():
    ...
    with open(accountsFile, 'w') as fp:
        fp.write("name,sent\n")
        for i in [x for j, x in enumerate(range(17)) if j != 4]:
            if i == 0:
...
def getEmail():
    """
    Reads the emails.csv to get the email account index with the least number of sent emails
...
def sendMail(curso, newestNews, mailList):
    ...
    for entry in newestNews:
        ...
        for mailList in [mailList[x:x + tmpNumb] for x in range(0, len(mailList), tmpNumb)]:
            ...
            yag = yagmail.SMTP(sender_email, "Edersemelhante123.!")
            body = entry[2], \
